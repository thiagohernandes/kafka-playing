...
@Configuration
public class BasicConsumerFirst {

    private final Logger LOGGER = LoggerFactory.getLogger(BasicConsumerFirst.class);
    ...
    @Autowired
    public BasicConsumerFirst(final BasicExceptionHandler exceptionHandler) {
    ...
    public void init() {
        try (var consumer = new KafkaConsumer<String, String>(CONFIG)) {
            ...
            for (var record : records) {
                LOGGER.info("[ConsumerBasicOne] Valor recebido {}", record.value());
            }
            consumer.commitAsync();
        } catch(Exception e) {
            exceptionHandler
                    .handleExceptionKafka(LOGGER, BasicConsumerFirst.class.getSimpleName() , e);
        }
    }
}
